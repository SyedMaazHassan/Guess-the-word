...
LIST=['BITE', 'BEAT', 'COME', 'EDIT', 'HOLD', 'FAIL', 'TALK', 'WALK','FIND']
...
def wow():
    ...
    #        if chances!=0:
        ...
        if edit_label_1['text']==answer[0] and edit_label_2['text']==answer[1] and edit_label_3['text']==answer[2] and edit_label_4['text']==answer[3]:
            if ROUND<5:
                incriment()
                ...
                tkinter.messagebox.showinfo('Congratulations', "You have completed level 3. Click 'OK' to go to next level")
                back.destroy()
                os.system('python simple4.py')
    ...
    label03=Label(frame01,background='#FFFFFF', text='LEVEL 3', font=('arial','35'))
    ...
    label06=Label(frame03, bg='#142d39', fg='#FFFFFF', text='Guess a word that contains only 4 alphabets (verbs ONLY)', font=('arial','13'))
    ...
    edit_label_4.grid(row=1, column=5, padx=(5,30))



    sub_frame02=Frame(frame02, height=1,  width=100, bg='#000000').grid(row=1, column=6)
